...
| `operator_agent_pod_ttl_expired_total` | counter | agent_kind, outcome, handoff | Two independent dimensions: `outcome` = `graceful`\|`force_deleted` (how the pod stopped), `handoff` = `agent`\|`synthetic`\|`none` (how continuation state was captured). `handoff=none` is the work-loss bucket |
