...
- **A merge conflict on resume no longer fails the boot.** Bootstrap resumes an existing remote task branch and merges the repo's base branch into it ([tatara-claude-code-wrapper#159](https://github.com/szymonrychu/tatara-claude-code-wrapper/pull/159)). A conflict used to abort the whole bootstrap - `Render` failed, the pod never became Ready, and the operator respawned it every ~5 minutes toward the residency cap, for zero turns. Now the conflict arm returns a nil error (`merge --abort` still runs, leaving a clean tree - never conflict markers published to the branch), and the finding is injected into the agent's global CLAUDE.md as the hard-requirement first job to resolve.
