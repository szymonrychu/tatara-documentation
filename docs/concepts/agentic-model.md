...
| Human review rounds (`review`-kind Task) | hardcoded, not a field | `5` | Bounds the `awaiting-human` <-> `awaiting-review` cycle on a human's own PR. Still enforced, but the `5` is the `MaxHumanReviewRounds` constant in the operator, not a `Project` key - writing `agent.maxHumanReviewRounds` is pruned silently |
