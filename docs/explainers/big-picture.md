...
2. **Whether to approve any work the agent should implement - proposed or human-filed alike.** The only approval action is a maintainer's comment on the issue: the clarify agent judges whether it approves and cites it (a comment id plus a verbatim quote), and the operator independently verifies the citation exists, its author is a verified non-bot maintainer, and the quote is genuinely there - not sticky once consumed. There is no requirement that the cited comment be the thread's most recent - reading whether a later comment withdraws an earlier approval is the agent's job. Nothing else releases the gate: not the issue reporter commenting, not the bot commenting. Labels are a write-only projection of that decision, never a way to grant it - see [Approval Gates](../operations/security/approval-gates.md).
