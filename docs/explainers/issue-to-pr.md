...
`change_significance` does not exist on the wire until this call, so this is also the first point where an auto-approved Issue's ceiling can be checked: if any live owned Issue was granted only through the `autoApproveMaxSignificance` carve-out, the operator compares the declared level against that ceiling and refuses the call with `over-auto-approve-ceiling` if it is over. A maintainer-cited approval carries no such ceiling.

Opening the merge request is itself gated on approval. `mr_write(action=open)` and this same `submit_outcome(action=submitted)` call refuse with `409 {reason: "approval-required"}` while any live owned Issue still carries no approval evidence, naming each blocking issue and what it needs. There is no path from an unapproved issue to a PR - work done before the gate grants cannot be carried forward by anything. See [Approval Gates](../operations/security/approval-gates.md#the-ship-gate-no-merge-request-can-carry-unapproved-work) for the mechanics.
