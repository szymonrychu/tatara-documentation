...
=== "Brainstorm"
    ...
    | `staleProposalDays` | `int` | `0` | **Declared but unconsumed.** Its godoc describes a staleness reaper over bot-authored proposals with no human engagement - positive an explicit window in days, `0` (unset) a default window, negative the opt-out - but no code reads the field, so no proposal is auto-closed at any age. It is in the CRD and applies cleanly, which is why setting it looks like it worked. |
    | `minSessionIntervalMinutes` | `int` | `12` | Floors the wall-clock gap between two brainstorm sessions, whichever path dispatched the prior one. A rate limit, not a circuit breaker: it delays a refill, never suppresses one, and it never inspects how the prior session ended. Positive is an explicit floor, `0` (unset) is the 12-minute default, negative disables the floor. Unlike `staleProposalDays` above, this one is live. |
