...
| `mergeable` | `bool` | Whether the forge reports the PR/MR as mergeable. **A different field from the `mergeable` in a `scm_read(kind=ci)` response**, which since [tatara-operator#609](https://github.com/szymonrychu/tatara-operator/issues/609) means no-conflicts AND not-red: GitLab's `merge_status` answers "are there conflicts" and says `can_be_merged` on a red pipeline |
