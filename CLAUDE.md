# CLAUDE.md - tatara-documentation

## What this repo is

Source for the [tatara platform documentation site](https://szymonrychu.github.io/tatara-documentation/):
mkdocs, the prose under `docs/`, and the design-doc archive under
`docs/appendix/design-docs/`.

## What this repo is NOT

- Not a code component. No Go, no Dockerfile, no chart. Markdown and mkdocs.
- Not the contract. The block below is generated from
  `tatara-agent-skills/template/CLAUDE-shared.md`; documenting a rule here does
  not change it.
- Not a live reference. `docs/appendix/design-docs/` is an ARCHIVE. Its
  documents record what was decided on their date, including rule citations
  that no longer resolve, and they are not swept when the contract changes.
...
- `scripts/check-stale-terms.sh` fails CI on retired vocabulary. Extend it when
  a term is retired; do not exempt a file to land a change.
- Design docs are written once and left alone. Corrections go in a NEW dated
  document that supersedes the old one, not as edits to history.
